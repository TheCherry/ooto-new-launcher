...
const logger = require(global.dirModloader + 'OotLogger')("Client");
const api = require(global.dirModloader + 'OotAPI');
const udp = require(global.dirModloader + 'OotUDP');
const fs = require('fs');
const version = require(global.dirModloader + 'OotVersion');
let websocket;
...
class Client {
    ...
    setup(server_ip) {
        (function (inst) {
            ...
            websocket = IO_Client.connect("http://" + server_ip + ":" + CONFIG.master_server_port);
            ...
            websocket.on('room', function (data) {
                logger.log(data.msg);
                let e = { id: "onServerConnection", ip: server_ip, port: CONFIG.master_server_port, room: CONFIG.GAME_ROOM, isModdedLobby: data.isModdedLobby };
            ...
            websocket.on('udp', function (data) {
                ...
                inst._upnp_client.portMapping({
                    ...
                    if (err) {
                        ...
                        logger.log("Sending UDP test to " + server_ip + ".")
                        inst._udp.sendTo(server_ip, data.port, { packet_id: "udpPunch", room: CONFIG.GAME_ROOM, uuid: CONFIG.my_uuid, nickname: CONFIG.nickname, punchthrough: inst._udp.port });
